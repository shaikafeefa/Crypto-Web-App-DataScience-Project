...
st.set_page_config(page_title="Crypto Web App", layout="wide")
...
st.markdown("""
This app retrieves prices and cryptocurrency analytics data.
""")
...
def load_data():
    ...
    for coin in coins:
        ...
        price.append(float(coin['price_usd']))
        percent_change_1h.append(float(coin['percent_change_1h']))
        percent_change_24h.append(float(coin['percent_change_24h']))
        percent_change_7d.append(float(coin['percent_change_7d']))
        market_cap.append(float(coin['market_cap_usd']))
        volume_24h.append(float(coin['volume24']))
    ...
    total_marketcap = sum(market_cap)

    btc = next((coin for coin in coins if coin['symbol'] == 'BTC'), None)
    eth = next((coin for coin in coins if coin['symbol'] == 'ETH'), None)

    btc_market_share = (
        float(btc['market_cap_usd']) / total_marketcap
    ) * 100

    eth_market_share = (
        float(eth['market_cap_usd']) / total_marketcap
    ) * 100
...
selected_coin_df = df[
    df['coin_symbol'].isin(selected_coin)
]
...
selected_percent_timeframe = percent_dict[
    percent_timeframe
]
...
top_5_positive_change = df.nlargest(
    5,
    selected_percent_timeframe
)

top_5_negative_change = df.nsmallest(
    5,
    selected_percent_timeframe
)

positive_change_selected_coins = selected_coin_df[
    selected_coin_df[selected_percent_timeframe] > 0
]

negative_change_selected_coins = selected_coin_df[
    selected_coin_df[selected_percent_timeframe] < 0
]

bar_chart_df = pd.concat([
    top_5_positive_change,
    positive_change_selected_coins,
    top_5_negative_change,
    negative_change_selected_coins
], axis=0)
...
col2.write(f'Last {percent_timeframe}')

fig, ax = plt.subplots(figsize=(8, 6))

ax.barh(
    bar_chart_df['coin_symbol'],
    bar_chart_df[selected_percent_timeframe],
    color=bar_chart_df['positive_percent_change'].map({
        True: 'green',
        False: 'red'
    })
)
...
col3.subheader('Market Cap of Selected Cryptos')

fig, ax = plt.subplots(figsize=(8, 6))

ax.bar(
    selected_coin_df['coin_symbol'],
    selected_coin_df['market_cap']
)
...
selected_crypto = col3.selectbox(
    'Select Crypto',
    df['coin_symbol']
)

col3.header(
    f'{selected_crypto} Price Over Last 30 Days'
)
...
historical_timeseries_df = (
    get_historical_time_series_data(
        selected_crypto,
        month_ago,
        today
    )
...
ax.plot(
    historical_timeseries_df['Date'],
    historical_timeseries_df['Close']
)
...
selected_coin_price_info_df = (
    selected_coin_df[columns]
)

col2.subheader(
    'Price Data of Selected Cryptocurrencies'
)

col2.dataframe(selected_coin_price_info_df)
...
col2.markdown(
    filedownload(selected_coin_price_info_df),
    unsafe_allow_html=True
)
